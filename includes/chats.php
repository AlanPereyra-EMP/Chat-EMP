...
function cemp_get_chat_list(){
  ...
  $user_list = $wpdb->get_results(
    "SELECT * FROM $table_chats, $table_usrs WHERE (`user_id1` = $user_id OR `user_id2` = $user_id) AND `user_id1` != `user_id2` GROUP BY `user_id1`"
  );
  ...
  for($i = 0; $i < $chats_number; $i++){
    ...
    $img = '<img src="'.$img_src.'"/>';
    $chats .= '<li onclick="cempGetThisChat(20,'.$user_list[$i]->user_id1.')">'.$img.$user_name.'</li>';
  }
